...
func load_master_database() -> void:
	...
	var loaded_pull_rates = AssetLoader.load_json_data("pullRates.json")
	pull_rates = loaded_pull_rates if loaded_pull_rates != null else {}
	
	var loaded_rarities = AssetLoader.load_json_data("rarities.json")
	rarities_db = loaded_rarities if loaded_rarities != null else {}
