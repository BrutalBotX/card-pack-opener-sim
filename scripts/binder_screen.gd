...
# CLEANUP: If the user changes scenes while a popup is open, force close it
...
func load_master_database() -> void:
	var raw_sets = AssetLoader.load_json_data("sets.json")
	if raw_sets:
		for series_key in raw_sets.keys():
			for s in raw_sets[series_key]:
				...
				if not set_names.has(display_code):
					...
					if display_code == "PROMO": set_representatives[display_code] = "Promo Pack"
					elif packs.size() > 0: set_representatives[display_code] = str(packs[0])
					else: set_representatives[display_code] = ""

	rarities_db = AssetLoader.load_json_data("rarities.json")
	var raw_cards = AssetLoader.load_json_data("cards.json")
	if raw_cards:
		card_db.clear()
		for card in raw_cards:
			...
			card_db[c_id] = {"name": card.get("name", "Unknown"), "rarity": str(card.get("rarity", "C")), "image": card.get("image", "")}
			var display_code = set_code 
			if display_code.to_upper().begins_with("PROMO"): display_code = "PROMO"
...
func build_set_selection_grid() -> void:
	...
	current_viewed_set = "" 
	for child in set_grid_container.get_children(): child.queue_free()
	for set_code in set_groups.keys():
		...
		set_btn.texture_normal = AssetLoader.get_pack_texture(set_representatives.get(set_code, ""))
		...
		if logo_texture != null:
			...
			name_label.text = set_code + " - " + set_names.get(set_code, "")
...
func build_binder_grid(set_code: String) -> void:
	for child in card_grid_container.get_children(): child.queue_free()
	load_inventory()
	if not set_groups.has(set_code): return
...
func spawn_3d_inspection_popup(c_id: String, card_data: Dictionary) -> void:
	if active_3d_popup != null: active_3d_popup.queue_free()
	...
	# FIX: Add to ROOT to ensure it covers everything including UI containers
	get_tree().root.add_child(active_3d_popup)
	...
	if card_instance.is_special_rare: card_instance.shine_particles.emitting = true
